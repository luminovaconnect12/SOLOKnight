    ...
    <footer className="relative border-t border-knight-blue/15 bg-knight-ink">
      ...
      <Reveal y={20} className="relative mx-auto max-w-7xl px-6 py-16 lg:px-10">
        <div className="grid gap-12 lg:grid-cols-[1.4fr_1fr_1fr_1fr]">
          <div>
            <p className="font-hud text-sm uppercase tracking-[0.3em] text-foreground">Solo <span className="text-knight-blue-bright">Knight</span></p>
          ...
          <div>
            ...
            <ul className="mt-4 space-y-3">
              {navLinks.map((link) => (<li key={link.href}><a href={link.href} className="text-sm text-knight-gray transition-colors hover:text-foreground">{link.label}</a></li>))}
            </ul>
          </div>
          <div>
            ...
            <ul className="mt-4 space-y-3 text-sm text-knight-gray">
              <li><a href={`mailto:${siteConfig.contactEmail}`} className="transition-colors hover:text-foreground">{siteConfig.contactEmail}</a></li>
          ...
          <div>
            ...
            <ul className="mt-4 space-y-3 text-sm text-knight-gray">
              <li><a href={siteConfig.social.instagram} className="transition-colors hover:text-foreground" target="_blank" rel="noreferrer">Instagram</a></li>
              <li><a href={siteConfig.social.x} className="transition-colors hover:text-foreground" target="_blank" rel="noreferrer">X / Twitter</a></li>
              <li><a href={siteConfig.social.youtube} className="transition-colors hover:text-foreground" target="_blank" rel="noreferrer">YouTube</a></li>
            </ul>
          </div>
        </div>
